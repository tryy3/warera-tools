...
async function createMemoryDb(): Promise<Db> {
  const dir = mkdtempSync(join(tmpdir(), "skills-route-"));
...
describe("GET /bootstrap", () => {
  ...
  it("returns skills bootstrap shape for a fixture user", async () => {
  ...
  it("accepts refresh=1", async () => {
    ...
    const res = await appFor(db, request).request("http://localhost/bootstrap?userId=u1&refresh=1");
    expect(res.status).toBe(200);
    const body = (await res.json()) as {
