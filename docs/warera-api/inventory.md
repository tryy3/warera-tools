...
| Item-market transactions | Equipment / itemMarket and commodity trading sales stream | `item-market-tx-backfill` (once per process) then `item-market-tx-poll` (both `itemMarket` + `trading` types; poll **deepens** `trading` toward chart max **30d** in budgets of ~40 pages/tick, resuming via job state cursor); **manual deep history:** `vp run backfill:item-market-tx -- --until <ISO> [--type …]` (operator-run, not scheduled) | Poll every minute; backfill `maxRuns: 1` | api2 + `X-API-Key` | Append-only (`item_market_transactions`) + handoff cursor; buyer/seller indexes for player lookups | Equipment Market (`/api/equipment` overview, detail, craft-compare), Battle quote (`POST /api/battle-build/quote`), Market my-trades |
| Commodity trading transactions | Stackable buy/sell fills | same `item-market-tx-*` jobs (both transaction types); deepen lookback = chart max (`30d`); manual backfill via same CLI (`--type trading` or `both`) | Poll every minute; backfill `maxRuns: 1` | api2 | `item_market_transactions` | Market my-trades |
...
The country watchlist is distinct ids in `country_watch_reasons` (Sweden seeded manually today); full country catalog still comes from `country-sync` into `countries`.
