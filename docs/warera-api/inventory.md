...
| MU member fight state | Combat stats, resources, equipment, pill state, and skills for watched MU rosters | `mu-fight-poll` | Every 5 minutes | `user.getUserById` batch | Append-on-change `user_fight_polls` / `user_fight_snapshots` (poll row every run; snapshot only when content changes) | Fight Desk |
