...
| Item-market transactions | Equipment / itemMarket sales stream | `item-market-tx-backfill` (once per process) then `item-market-tx-poll` | Poll every minute; backfill `maxRuns: 1` | api2 + `X-API-Key` | Append-only (`item_market_transactions`) + handoff cursor | Equipment Market (`/api/equipment` overview, detail, craft-compare), Battle quote (`POST /api/battle-build/quote`) |
